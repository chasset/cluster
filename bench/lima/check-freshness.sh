...
#
# Usage :
#   bench/lima/check-freshness.sh
#   SEUIL_STORAGE_REAL=45 bench/lima/check-freshness.sh
#
...
if [ ! -f "${HISTORY}" ] || [ -z "$(metro_last_date < "${HISTORY}" || true)" ]; then
    ...
    if [ -z "${last_iso}" ]; then
        ...
        echo "Aucun run consigné — lancer 'bench/lima/run-phases.sh atlas' et committer la preuve."
        exit 2
    fi
