...
#
# Usage :
#   bench/lima/env.sh            # auto-détecte (banc Lima si des VMs existent, sinon prod)
#   bench/lima/env.sh lima       # force le contexte banc Lima
#   bench/lima/env.sh prod       # force le contexte prod (bootstrap/hosts.yaml)
...
if [ "${mode}" = auto ]; then
    if [ -n "${has_lima}" ] && [ -n "${has_prod}" ]; then
        ...
        printf '  Préciser la cible : %sbench/lima/env.sh lima%s  ou  %sbench/lima/env.sh prod%s\n' \
            "${C}" "${N}" "${C}" "${N}" >&2
        exit 2
...
if [ "${mode}" = lima ]; then
    hosts=$(lima_hosts | tr '\n' ' ')
    if [ -z "${hosts# }" ]; then
        printf '  %sAucune VM Lima en cours. Monter le banc : %sbench/lima/run-phases.sh up%s\n' "${D}" "${N}${C}" "${N}"
        exit 0
    fi
    ...
    printf '  %sbench/lima/run-phases.sh status%s   %s← vue d'\''ensemble VMs+nœuds+phases+UIs%s\n' "${C}" "${N}" "${D}" "${N}"
    printf '\n%sPiloter le cluster (kubectl)%s\n' "${B}" "${N}"
    if [ -f "${KUBECONFIG_LOCAL}" ]; then
        # Exemple littéral à copier (le `$(...)` ne doit PAS s'expandre ici).
        # shellcheck disable=SC2016
        printf '  %seval "$(bench/lima/env.sh export)"%s   %s← exporte KUBECONFIG=%s%s\n' \
            "${C}" "${N}" "${D}" "${KUBECONFIG_LOCAL}" "${N}"
    else
        printf '  %skubeconfig absent — %sbench/lima/run-phases.sh kubeconfig%s\n' "${D}" "${C}" "${N}"
    fi
else
