...
# qui sont testées par bats (bench/unit/metrology.bats) sans monter de VM.
#
# Artefacts :
#   - bench/lima/runs-history.yaml  : VERSIONNÉ (preuve datée, ADR 0042/0034).
#                                    Une entrée append par run `all` complété.
#   - bench/lima/.work/metrics.txt  : éphémère, en-tête matériel + durées (existant).
#
...
# Chemin du fichier d'historique versionné (sous bench/lima/, pas .work/).
...
#
# Une entrée est appendée par run `all` COMPLÉTÉ (bench/lima/run-phases.sh). Les
...
metro_socle_key() {
    ...
    h=$(git -C "${REPO}" ls-files -z -- \
            ...
            'bench/lima/profiles/node.yaml.tmpl' \
