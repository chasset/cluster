...
# montée (par `bench/lima/run-phases.sh dataops-chain`, qui déploie monitoring →
...
if [ "$chain_present" != "1" ]; then
    if [ "$STRICT_OL" = "1" ]; then
        ...
        log "  Attendu après 'bench/lima/run-phases.sh dataops-chain'."
        exit 1
    fi
    log "skip — chaîne DataOps non montée (Marquez et/ou Dagster absents)."
    log "  Monter la chaîne d'abord : bench/lima/run-phases.sh dataops-chain"
