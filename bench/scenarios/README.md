...
| 28  | UI joignables via Gateway           | HTTPRoute réellement posés → chaque hostname répond à travers le Gateway Cilium (SNI + TLS de bordure, multiplexage 443) ; skip si aucun HTTPRoute                 | ~2 min        | **Intégration** exposition UI (Gateway/cert-manager, ADR 0071/0021, #232)  |
...
| 30  | ha-3cp — survie d'1 panne CP        | Arrêt du CP **porteur de la VIP** → kube-vip **bascule** (VIP répond toujours) + **quorum etcd 2/3** + API joignable ; restore → 3/3 (topologie `ha-3cp` requise)  | ~3 min        | **HA control-plane** : VIP + quorum survivent à 1 CP (ADR 0047/0055, #250) |
