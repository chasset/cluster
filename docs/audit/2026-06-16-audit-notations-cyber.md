...
| ------------------------ | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Branch-Protection`      | **partiel**        | **erreur de lecture** Scorecard (« Resource not accessible by integration ») : lire la protection exige des droits admin que le `GITHUB_TOKEN` n'a pas → `repo_token: SCORECARD_TOKEN` ajouté (`scorecard.yml`). RÉEL (`gh api …/branches/main/protection` 2026-06-19) : 13 required checks + `strict:true` **mais** `reviews:0` (mono-mainteneur), `enforce_admins:false`, `linear:false` (voulu — merge-commit ADR 0037), `signatures:false`. La prédiction « vert attendu » du 2026-06-16 était **inexacte** (token non lisible alors)                                  |
| `Security-Policy`        | **vert**           | `SECURITY.md` présent (Private Vulnerability Reporting + modèle de menace assumé)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `Pinned-Dependencies`    | **vert (partiel)** | actions GitHub 100 % SHA-pinnées ([ADR 0006](../decisions/0006-matrice-de-versions-et-politique-de-bump.md)) ; **images** : Dagster (#434), MLflow et désormais Marquez (`eclipse-temurin:17`, `node:18-alpine`) épinglées par digest d'index multi-arch (banc arm64). **Restent NON pinnés (assumé)** : `pip install`/`npm install` dans les images de build internes — le hash-pinning (`--require-hashes` + fichiers de hashes ; `npm ci` + lockfile) est un effort élevé pour un gain net faible sur des images reconstruites en interne (ADR 0061) ; tracé, non câblé |
...
| `Packaging`              | **rouge (assumé)** | aucun paquet publié (npm/PyPI/registry). `nestor` est du Python qui **pilote CE dépôt** (chemins relatifs `bench/`, `topologies/`, `run-phases.sh` ; `pyproject` : « pas un paquet distribuable », ADR 0017) : un `pip install` hors dépôt n'aurait rien à piloter. Publier pour le badge = badge décoratif, écarté (ADR 0061/0080). N/A **assumé et tracé**, comme `Code-Review` — pas un trou                                                                                                                                                                            |
...
**Restent assumés** : `Code-Review` (mono-mainteneur, SAFEGUARDS.md),
`Maintained` (dépôt < 90 j, se résout seul), `Packaging` (N/A tracé : `nestor`
pilote ce dépôt et n'a pas de sens en paquet autonome — cf. ligne du tableau
ci-dessus). `Fuzzing` et `Signed-Releases` sont **sortis** de cette liste :
property-based testing et signature de release en cours (ADR 0087/0088).
