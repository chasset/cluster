...
| `cluster-dataops` | sur demande     | — (déclenché par risque)  | **23–26** (DataOps + observ.)  | **Amendé par [ADR 0085](0085-preuves-applicatives-local-path.md)** : la chaîne applicative sur Ceph ne tient pas en ressources au banc dev → la preuve applicative de référence est `atlas` (local-path). `cluster-dataops` reste la **seule** preuve du « RGW rempli par un workload » (backups CNPG/Loki/MLflow → RGW) ; joué **avant tout changement S3/backing/storageClass** (soupape ADR 0036), plus à cadence calendaire. |

> **[ADR 0085](0085-preuves-applicatives-local-path.md)** amende cette matrice :
> les preuves **applicatives** e2e se font sur `atlas` (local-path) par défaut ;
> Ceph est validé sur son **installation seule** (`storage-real`) ;
> `cluster-dataops` (applicatif sur Ceph) passe d'« optionnel 90 j » à « sur
> demande, avant un changement touchant le chemin S3/stockage ».
