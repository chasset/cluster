...
1. **`NODES` (bash, `test/lima/run-phases.sh`)** — nœuds/rôles du banc, **codés
...
- **(8) Métriques** exportées (existent déjà — `test/lima/metrology.sh`) :
...
**Consigner (historique des runs — `test/lima/runs-history.yaml`, existe)**
...
`test/RESULTS.md`, **jamais** par une variable `inject_fault` dans le rôle prod.
