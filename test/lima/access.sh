...
# isolée en fonctions testables (test/unit/access.bats), sans cluster.
#
...
#
# Usage :
#   test/lima/access.sh            # pose tout + affiche URLs/secrets + génère .env
#   test/lima/access.sh --stop     # arrête les forwards, retire le bloc /etc/hosts
#   test/lima/access.sh --print-hosts   # imprime le bloc /etc/hosts (pas de sudo)
#   test/lima/access.sh --no-hosts      # tout sauf /etc/hosts (URLs en --resolve)
set -euo pipefail
...
HOSTS_TAG="cluster.lan (banc Lima — test/lima/access.sh)"
...
generate_env() {
    ...
    {
        echo "# Généré par cluster/test/lima/access.sh — NE PAS COMMITER (gitignoré)."
